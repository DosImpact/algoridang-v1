...
const MonoTickerSettingButton: React.FC<IMonoTickerSettingButton> = ({
  ...
  const thisCorp = thisUnivSetting?.selectedCorporations;
  console.log('thisCorp', thisCorp);
  ...
  const [, setCorporation] = useRecoilState(atomCorporationState);
  const handleChangeTickerPriceChart = () => {
    if (thisCorp) setCorporation(thisCorp);
  };
