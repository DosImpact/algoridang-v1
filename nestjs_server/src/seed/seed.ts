...
async function bootstrap() {
  ...
  const strategyTC_01 = () => {
    if (getCorporations.corporations) {
      getCorporations.corporations.slice(100).map(async (corp) => {
        ...
        const newStrategy = await strategyService.createMyStrategy(
          ...
          {
            investProfitInfo: {
              securities_corp_fee: '0.03',
              invest_start_date: '2007-01-01T06:58:48.421Z',
              invest_end_date: '2010-12-30T06:58:48.421Z',
              invest_principal: '10000000',
            },
            strategy_explanation: `기본 골든 크로스 전략 입니다.(${corp_name},${ticker}) 금융위기(2007~2010)년 기간 테스트 입니다.`,
            strategy_name: `${corp_name} 골든 크로스 전략(5,15)`.slice(0, 29),
            open_yes_no: false,
            tags: ['기본전략', '골든크로스'],
          },
        );
        // 유니버셜 추가
        await tradingService.addUniversal('Rich@Rich.Rich', {
        ...
        // 백테스팅 요청
        await flaskService.pushBackTestQ('Rich@Rich.Rich', {
          strategy_code: newStrategy.memberStrategy.strategy_code,
        });
